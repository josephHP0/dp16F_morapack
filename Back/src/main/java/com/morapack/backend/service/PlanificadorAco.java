...
// Importar clases de Dominio/Modelo
...
// Importar clases auxiliares de la misma capa de servicio/núcleo
import com.morapack.backend.service.GrafoVuelos;
import com.morapack.backend.service.Asignacion;
import com.morapack.backend.service.Ruta;
import com.morapack.backend.service.ParametrosAco;

// Importar UtilArchivos (asumo que está en util)
...
public class PlanificadorAco {
    ...
    // NOTA: Esta clase auxiliar se mantiene como inner-class en PlanificadorAco, lo cual es correcto.
    ...
    public static Map<Integer, Set<Integer>> mapearCancelacionesAIdsPorDia(
        ...
        for (Vuelo v : vuelos) {
            // Usamos la capacidad original (capacidadMax) si la implementaste en Vuelo
            String key = v.getOrigen() + "|" + v.getDestino() + "|" + v.getSalidaMin();
            idx.computeIfAbsent(key, k -> new ArrayList<>()).add(v.getId());
        }
    ...
    private static Ruta construirRuta(String hub, String destino,
        ...
        for (int s = 0; s < pasosMax && horas <= presupuestoHoras; s++) {
            ...
            for (var e : aristas) {
                ...
                // Cálculo del tiempo de llegada al aeropuerto actual (en horas del día)
                // Se necesita la hora de llegada al aeropuerto para calcular el tiempo de espera.
                // El tiempo transcurrido total es 'horas'
                int minutosTranscurridosDesdePedido = horaInicio * 60 + minutoInicio + (int) Math.round(horas * 60);
                int diasTranscurridos = minutosTranscurridosDesdePedido / (24 * 60);

                int minutoDelDiaLlegada = minutosTranscurridosDesdePedido % (24 * 60);
                ...
                int salida = vuelo.getSalidaMin();
                int minutosEspera;
                int diaSalida = diaActual; // Inicialmente asumimos que sale hoy

                if (salida >= minutoDelDiaLlegada) {
                    minutosEspera = salida - minutoDelDiaLlegada;
                } else {
                    // espera hasta mañana
                    minutosEspera = (24 * 60 - minutoDelDiaLlegada) + salida;
                    diaSalida = diaActual + 1; // El vuelo saldrá mañana
                }

                // Si el vuelo sale mañana, validamos la cancelación de mañana
                if (diaSalida > diaActual) {
                ...
                // Si la suma de tiempo (horas + espera + vuelo) supera el presupuesto, descartamos
                double tiempoTotalConEspera = horas + (minutosEspera / 60.0) + elegido.horas;
                if (tiempoTotalConEspera > presupuestoHoras) continue;
                ...
                // Los parámetros alpha y beta deben venir de ParametrosAco
                double alphaParam = p.alpha;
                double betaParam = p.beta;
                double score = Math.pow(Math.max(tauVal, eps), alphaParam) * Math.pow(Math.max(heurVal, eps), betaParam);
                candidatos.add(e);
                pesos.add(score);
            }
            ...
            int salida = v.getSalidaMin();
            double esperaHoras;
            if (salida >= minutoDelDiaLlegada) {
                esperaHoras = (salida - minutoDelDiaLlegada) / 60.0;
            } else {
                esperaHoras = ((24 * 60 - minutoDelDiaLlegada) + salida) / 60.0;
            }
            ...
            ruta.itinerario.add(actual + "->" + elegido.destino + " (" + String.format(Locale.US, "%.2f", elegido.horas) + "h)");
    ...
    public static List<Asignacion> planificarConAco(
        ...
        for (Aeropuerto ap : aeropuertos.values()) {
            if (ap.getContinente() != null && !ap.getContinente().isBlank()) {
                REGION_BY_IATA.put(ap.getCodigo(), ap.getContinente());
            }
        }

        // Crear mapa de vuelos por ID para acceso rápido
        Map<Integer, Vuelo> vuelosPorId = new HashMap<>();
        for (Vuelo v : vuelos) vuelosPorId.put(v.getId(), v);

        GrafoVuelos grafo = new GrafoVuelos(vuelos);
        double[] tau = new double[vuelos.size() + 1]; // +1 para índice base 1
        double[] heur = new double[vuelos.size() + 1]; // +1 para índice base 1
        Arrays.fill(tau, 0.1);

        // Heurística basada en distancia, duración y capacidad disponible (inicial)
        for (Vuelo v : vuelos) {
            Aeropuerto a1 = aeropuertos.get(v.getOrigen());
            Aeropuerto a2 = aeropuertos.get(v.getDestino());
            double heurVal = 1e-6;
            if (a1 != null && a2 != null) {
                double dist = UtilArchivos.distanciaKm(a1, a2);
                double durHoras = v.getHorasDuracion();
                ...
                // Usamos capacidadMax del nuevo Vuelo (asumiendo que tiene un getter)
                heurVal *= Math.max(1.0, v.getCapacidadMax() / 100.0);
            }
            heur[v.getId()] = heurVal;
        }

        // Capacidad restante (reset por planificación)
        Map<Integer,Integer> capRest = new HashMap<>();
        // Inicializamos con la capacidad disponible (capacidad) del objeto Vuelo clonado
        for (Vuelo v : vuelos) capRest.put(v.getId(), v.getCapacidad());
        ...
        for (Pedido ped : pedidos) {
            String hub = hubParaDestino(ped.getDestinoIata());
            double presupuesto = slaHoras(hub, ped.getDestinoIata());

            Ruta mejor = null;
            for (int it=0; it<p.iteraciones; it++) {
                for (int h=0; h<p.hormigas; h++) {
                    Ruta r = construirRuta(
                            hub, ped.getDestinoIata(), grafo, tau, heur, p.pasosMax, presupuesto,
                            capRest, aeropuertos, vuelosPorId,
                            ped.getDia(), ped.getHora(), ped.getMinuto(),
                            cancelByDay,
                            rnd
                    );
            ...
            int paquetesRestantes = ped.getPaquetes();
            // Permitir hasta 3 rutas alternativas por pedido
            int intentosRuta = 0;
            // Clonamos la capacidad al inicio de cada intento de ruta.
            Map<Integer, Integer> capRestActual = new HashMap<>(capRest);

            while (paquetesRestantes > 0 && mejor != null && intentosRuta < 3) {
                intentosRuta++;

                // Realiza la busqueda de la mejor ruta nuevamente para la capacidad restante
                if (intentosRuta > 1) {
                    Ruta mejorAlt = null;
                    double mejorHoras = Double.POSITIVE_INFINITY;
                    // Resetear la feromona para la búsqueda alternativa para evitar bucles.
                    Arrays.fill(tau, 0.1);
                    for (int it=0; it<5; it++) {
                        for (int h=0; h<15; h++) {
                            Ruta r = construirRuta(
                                    hub, ped.getDestinoIata(), grafo, tau, heur, p.pasosMax, presupuesto,
                                    capRestActual, aeropuertos, vuelosPorId,
                                    ped.getDia(), ped.getHora(), ped.getMinuto(),
                                    cancelByDay,
                                    rnd
                            );
                ...
                // Si encontramos una ruta, procedemos a la asignación
                ...
                // Almacén destino
                Aeropuerto apDest = aeropuertos.get(ped.getDestinoIata());
                int cuelloAlmacen = Integer.MAX_VALUE;
                if (apDest != null) {
                    int minutoLlegada = ped.getDia() * 24 * 60 + ped.getHora() * 60 + ped.getMinuto() + (int)(mejor.horasTotales * 60);
                    int ocupMax = 0;
                    for (int m = minutoLlegada; m < minutoLlegada + 120; m++) {
                        ocupMax = Math.max(ocupMax, apDest.ocupacionPorMinuto.getOrDefault(m,0));
                    }
                    cuelloAlmacen = Math.max(0, apDest.getCapacidad() - ocupMax); // Usamos getCapacidad()
                }
                ...
                // La lógica de buscar una ruta alternativa ya está dentro del bucle principal
                if (paquetesRestantes == 0) break;
            }
        }
        return resultado;
    }
    ...
    public static List<Asignacion> simularSemana(
        ...
        pedidos.sort(Comparator
                .comparingInt((Pedido p) -> p.getDia())
                .thenComparingInt(p -> p.getHora())
                .thenComparingInt(p -> p.getMinuto()));
        ...
        for (int d = 0; d < numeroDias; d++) {
            ...
            for (Pedido p : pedidos) if (p.getDia() == diaSim) pedidosDelDia.add(p);
    ...
    private static List<Vuelo> clonarVuelosConCapacidad(List<Vuelo> vuelos) {
        ...
        for (Vuelo v : vuelos) {
            // Se usa el constructor de Vuelo para crear una copia
            copia.add(new Vuelo(
                    v.getId(), v.getOrigen(), v.getDestino(), v.getSalidaMin(), v.getLlegadaMin(),
                    // Asumimos que v.getCapacidadMax() devuelve la capacidad total del vuelo
                    v.getCapacidadMax(), (int) Math.round(v.getHorasDuracion() * 60), v.isEsContinental()
            ));
        }
        return copia;
    }
